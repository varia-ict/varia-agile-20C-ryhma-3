...
        _x = gameObject.transform.localScale.x;
        ...
        //updates the scale of object by x y z added with growpertime which is now 0.3
        gameObject.transform.localScale = new Vector3(
          x: _x + GrowPerTime,
          y: _y + GrowPerTime,
          z: _z + GrowPerTime
        );
        //if x is less than maxscale it will start the growplant in order to grow to the full size
        if (_x < xMaxScale)
        {
